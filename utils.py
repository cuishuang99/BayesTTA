...
import numpy as np
import clip
from datasets.low_light_cifar10 import LowLightCIFAR10
from datasets.fmow import FMoW
from datasets.yearbook import Yearbook
from datasets.rmnist import RMNIST
...
def build_test_data_loader(dataset_name, root_path, preprocess,batch_size):
    if dataset_name == 'I':
        ...
        print(test_loader)
    elif 'low_light_cifar10' in dataset_name:
        degree = dataset_name[17:]
        dataset = LowLightCIFAR10(root_path, degree, preprocess)
        test_loader = torch.utils.data.DataLoader(dataset.test, batch_size=batch_size, num_workers=8, shuffle=True)
    elif 'fmow' in dataset_name:
        env = dataset_name[4:]
        dataset = FMoW(root_path, int(env), preprocess)
        test_loader = torch.utils.data.DataLoader(dataset.test, batch_size=batch_size, num_workers=8, shuffle=True)
    elif 'yearbook' in dataset_name:
        env = dataset_name[8:]
        dataset = Yearbook(root_path, int(env), preprocess)
        test_loader = torch.utils.data.DataLoader(dataset.test, batch_size=batch_size, num_workers=8, shuffle=True)
    elif 'rmnist' in dataset_name:
        env = dataset_name[6:]
        dataset = RMNIST(root_path, int(env), preprocess)
        test_loader = torch.utils.data.DataLoader(dataset.test, batch_size=batch_size, num_workers=8, shuffle=True)
