...
def initialize_state(clip_weights, args):
    ...
    return {
        ...
        'cfg': {
            ...
            'sigma': args.sigma,
            'alpha': args.alpha,
            'use_gda': True,  # Default to using GDA under H0
        },
    }
...
def compute_loss(logits_zs, logits_adapted):
    ...
    # Target distribution: softmax probabilities of logits_adapted (detach gradients)
    target_probs = F.softmax(logits_adapted.detach(), dim=1)  # q ∈ R^{B×K}
    
    # Compute cross-entropy: -Σ(q * log p)
...
def initialize_ema_params(clip_model, backbone):
    """Initialize EMA parameters of LN or BN layers based on backbone type."""
    ema_params = {}
...
def apply_ema(clip_model, ema_params, beta=0.99):
    """Apply EMA updates to LN or BN layer parameters."""
    with torch.no_grad():
        for name, param in clip_model.named_parameters():
...
def get_ln_bn_params(clip_model, backbone):
    """Retrieve LN or BN layer parameters based on backbone type."""
    if backbone == 'RN50':
...
def _covariance_homogeneity_test(x, P_zs, K, alpha=0.05):
    """Perform covariance homogeneity test based on the first batch of samples and class probabilities."""
    # ================== Data preprocessing ==================
    device = x.device
    B, D_orig = x.shape
    
    # PCA to 10 dimensions (on CPU)
    pca = PCA(n_components=10)
    x_lowdim_np = pca.fit_transform(x.cpu().numpy())  # [B, 10]
    x_lowdim = torch.tensor(x_lowdim_np, dtype=torch.float32, device='cpu')  # Keep as CPU tensor
    D = 10
    ...
    # ================== F-distribution parameter calculation ==================
    try:
...
def test_time_adaptation_em(images, clip_model, clip_weights, state_dict):
    """Stable version supporting GDA/QDA"""
    device = images.device

    # Unpack state parameters (handle dimensions automatically based on mode)
    mu_prev = state_dict['mu']  # [K,D]
    D = state_dict['D']
    K = state_dict['K']
    cfg = state_dict['cfg']

    # Trigger covariance homogeneity test on first run
    if not state_dict.get('_initialized', False):
        with torch.no_grad():
            ...
            # Perform test and set mode
            reject_h0 = _covariance_homogeneity_test(x, P_zs, K)
            cfg['use_gda'] = not reject_h0  # Use QDA if H0 is rejected, otherwise use GDA
            print("Use Shared Covariance (GDA) cfg['use_gda']", cfg['use_gda'])
            state_dict['_initialized'] = True  # Mark as initialized

    # Handle covariance-related parameters based on mode
    if cfg['use_gda']:
        # GDA mode parameters
    ...
    with torch.no_grad():
        ...
        # ================== Covariance Update (Branch by Mode) ==================
        diff = x.unsqueeze(1) - mu_prev.unsqueeze(0)  # [B,K,D]

        if cfg['use_gda']:
            # GDA mode: global covariance computation
            weighted_diff = torch.einsum('bk,bkd,bke->de', P_zs, diff, diff)  # [D,D]
            sigma_sum_new = sigma_sum_prev + weighted_diff
            sigma_count_new = sigma_count_prev + x.size(0)

            # Regularization and broadcasting
            sigma_regs = (sigma_sum_new / sigma_count_new) * (1 - cfg['epsilon']) + \
        ...
        # ================== Discriminant Function (Unified Handling) ==================
        sigma_invs = torch.linalg.pinv(sigma_regs)
        if sigma_regs.dim() == 2:
            sigma_invs = sigma_invs.unsqueeze(0).repeat(K, 1, 1)
            sigma_regs = sigma_regs.unsqueeze(0).repeat(K, 1, 1)
        diff_mu = x.unsqueeze(1) - mu_new.unsqueeze(0)  # [B,K,D]
        quad_term = -0.5 * torch.einsum('bkd,kde,bke->bk', diff_mu, sigma_invs, diff_mu)

        # Determinant computation (optimized for stability)
        log_dets = []
        for s in sigma_regs:
            try:
                ...
                log_det = torch.linalg.slogdet(s)
...
def adaptation_step(images, clip_model, clip_weights, state_dict, ema_params, optimizer, args):
    """Single adaptation step with EMA update"""
...
def main():
    ...
    save_file = f"{args.output}cd_alpha{args.alpha}_lr{args.learning_rate}_ema{args.model_ema_beta}.csv"
    os.makedirs(os.path.dirname(save_file), exist_ok=True)  # Ensure output directory exists

    with open(save_file, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Set-ID', 'Accuracy'])

        overall_acc = 0
        overall_samples = 0

        # Get dataset list (adjust according to parsed parameters)
        datasets = args.datasets.split('/')  # Assume '/' as separator
        ...
        for dataset_name in datasets:
            ...
            test_loader, classnames, template = build_test_data_loader(
                dataset_name, args.data_root, preprocess, args.batch_size
            )
            ...
            optimizer = torch.optim.Adam(ln_bn_params, lr=args.learning_rate)  # Small LR, momentum

            # Initialize global state and statistics across all domains
            domain_correct = 0
            domain_samples = 0
            ...
            writer.writerow([dataset_name, domain_acc])
            print(f"{dataset_name}: {domain_acc} (Samples: {domain_samples})")

            # Accumulate total accuracy and samples
            overall_acc += domain_acc
            overall_samples += domain_samples

        # Compute and write weighted average
        average_acc = overall_acc / 10
        writer.writerow(['Average', average_acc])
        print(f"Overall Average Accuracy: {average_acc}, Total Samples: {overall_samples}")
