...
def initialize_state(clip_weights, args):
    ...
    return {
        ...
        'cfg': {
            ...
            'sigma': args.sigma,
            'alpha': args.alpha,
            'use_lda': True,  # Default to using GDA under H0
        },
    }
...
def test_time_adaptation_em(images, clip_model, clip_weights, state_dict):
    ...
    if not state_dict.get('_initialized', False):
        with torch.no_grad():
            ...
            cfg['use_lda'] = not reject_h0  # Use QDA if H0 is rejected, otherwise use GDA
            print("Use Shared Covariance (GDA) cfg['use_lda']", cfg['use_lda'])
...
def main():
    ...
    save_file = f"{args.output}cd_{args.datasets}_alpha{args.alpha}_lr{args.learning_rate}_ema{args.model_ema_beta}.csv"
